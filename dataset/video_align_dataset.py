...
class VideoAlignmentTrainDataset(VideoAlignmentDataset):
    ...
    def __getitem__(self, idx):
        ...
        for video in selected_videos:
            ...
            frames = np.array(frames)  # (32, 224, 224, 3)
            final_frames.append(
...
class VideoAlignmentDownstreamDataset(VideoAlignmentDataset):
    ...
    def _construct_frame_path(self):
        ...
        for video in self.video_paths1:
            ...
            for frame_id in range(video_frames_count):
                temp_id_list.append(frame_id)
                temp_label_list.append(labels[frame_id])
                if view == 'ego':
    ...
    def __getitem__(self, idx):
        total_frame = self.video_len_list[idx]
        
        video_path = self.video_name_list[idx]
        frame_id = self.frame_id_list[idx]
        frame_label = self.label_list[idx]
        h5_file_name = _extract_frames_h5py(video_path, self.frame_save_path)
        frame = self.get_frames_h5py(h5_file_name, frame_id)
